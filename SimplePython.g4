...
// The following code for INDENT and UNINDENT tokens was made with assistance from the README of of
// Antlr Denter Helper, an open source project to help handle indentation Link:
...
statement: call_function_statment | assignment | comment_line;

statement_in_loop: break_statment | continue_statement;

comment_line: COMMENT NEWLINE;

// different blocks are required, because when a statement makes a new scope, it can use different
// statments. Like only inside a function there can be a return, and a loop can be a break. This is
// necessary so, for example, an if statment inside a function can return.

indentedBlock: INDENT block UNINDENT;

indentedBlockInLoop: INDENT blockInLoop UNINDENT;

indentedBlockInFunction: INDENT blockInFunction UNINDENT;
...
list_index: VAR ('[' expression ']')+;

iterable: list | tuple | call_function | STRING | VAR;

//function implementation
create_function:
	DEF SPACE+ VAR SPACE* '(' SPACE* parameters? SPACE* ')' SPACE* COLON SPACE* COMMENT?
...
if_statement:
	IF SPACE+ condition_block (ELIF SPACE+ condition_block)* (
		ELSE SPACE* COLON SPACE* COMMENT? indentedBlock
	)?;

if_statement_in_loop:
	IF SPACE+ condition_block_in_loop (
		ELIF SPACE+ condition_block_in_loop
	)* (ELSE SPACE* COLON SPACE* COMMENT? indentedBlockInLoop)?;

if_statement_in_function:
	IF SPACE+ condition_block_in_function (
		ELIF SPACE+ condition_block_in_function
	)* (
		ELSE SPACE* COLON SPACE* COMMENT? indentedBlockInFunction
	)?;

if_statement_in_function_and_loop:
	IF SPACE+ condition_block_in_function_and_loop (
		ELIF SPACE+ condition_block_in_function_and_loop
	)* (
		ELSE SPACE* COLON SPACE* COMMENT? indentedBlockInFunctionAndLoop
	)?;

while_loop: WHILE SPACE+ condition_block_in_loop;
...
for_loop_in_function:
	FOR SPACE+ VAR SPACE+ IN SPACE+ iterable SPACE* COLON SPACE* COMMENT?
		indentedBlockInFunctionAndLoop;
...
break_statment: BREAK SPACE* COMMENT? NEWLINE;

continue_statement: CONTINUE SPACE* COMMENT? NEWLINE;
...
// this has to be under other identifiers so they can take effect Variable starts with letter or
// underscore with n letters, numbers, and underscores after
